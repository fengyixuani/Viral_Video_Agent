...
class InputBundle:
    ...
    pure_music: bool = False  # 参考视频纯音乐无口播 → 强制镜头复刻(shot) + 静音 + 复用BGM
    missing_shot_mode: str = "aigc"  # 缺失镜头处理：aigc=生成 / skip=跳过 / match=匹配最相关
...
def load(payload) -> InputBundle:
    ...
    return InputBundle(
        ...
        missing_shot_mode=_string(payload, "missing_shot_mode", "aigc") or "aigc",
    )
