...
def extract_shot_thumbs(video_uri: str, shot_slots):
    ...
    优先用每个 shot 的**真实 source_time_range**（理解阶段 VLM 给出的起止秒）取该区间中点抽帧，
    保证缩略图与该镜的文字/breakdown 描述**同源**、不错位。缺 source_time_range 的镜头（老数据/
    模型没给）才退回"按声明时长等比缩放取中点"的估算，并只对这些镜头累计估算时间。
    """
    local = _resolve_local_path(video_uri)
    ...
    for shot in shot_slots:
        start, end = _parse_range(shot.get("source_time_range", ""))
        if end > start:
            # 真实区间中点（同源，最准）
            timestamp = (start + end) / 2.0
        else:
            # 兜底：按缩放后的声明时长累计取中点（旧路径，可能有累计漂移）
            d = float(shot.get("duration") or 0.0) * scale
            timestamp = accumulated + d / 2.0
            accumulated += d
        ts = max(0.05, min(duration - 0.05, timestamp))
        # 文件名带上抽帧时间点：换了 source_time_range（新的正确时间）就会生成新文件，
        # 不会复用旧的、按估算时长抽出的错位缩略图。
        name = f"{stem}_shot{int(shot.get('id') or 0):02d}_{int(round(ts * 1000))}.jpg"
