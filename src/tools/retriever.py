...
def _segment_text(seg: dict) -> str:
    """语义索引文本：只用 summary + visual_description（口播默认排除）。"""
...
def queries_for_shot(shot: dict) -> list:
    """为一个 shot 生成多条互补 query：L1 意图（必生成）+ L2 视觉（有 breakdown 时）。"""
    ...
    if visual_vals:
        queries.append(" ".join(visual_vals))
    # 去重 + 去空
    seen, out = set(), []
    for q in queries:
        q = q.strip()
...
class Retriever:
    ...
    def index_segments(self, segments: list, source: str = "") -> dict:
        ...
        with _LOCK:
            ...
            added = skipped = 0
            for seg, text, vec in zip(segs, texts, vectors):
    ...
    def search(self, queries, top_k: int = None) -> dict:
        ...
        if not query_list or not records:
            return base
        ...
        try:
            ...
            for query, qvec in zip(query_list, qvecs):
                scored = [(r["id"], _cosine(qvec, r["vector"])) for r in records]
                scored.sort(key=lambda x: x[1], reverse=True)
                rankings.append([rid for rid, _ in scored])
                cosine_by_query[query] = {rid: round(sc, 4) for rid, sc in scored}
        except RuntimeError as exc:
            ...
            for query in query_list:
                scored = [
                    (r["id"], lexical_score(query, r.get("text", ""), r.get("meta", {}).get("keywords", [])))
                    for r in records
                ]
                scored.sort(key=lambda x: x[1], reverse=True)
                rankings.append([rid for rid, _ in scored])
