...
async def orchestrate_from_feasibility(reference_template: dict, feasibility: dict, *,
                                       ...
                                       intent="", shot_replicate=False,
                                       missing_shot_mode="aigc") -> AsyncGenerator[dict, None]:
    """基于素材可行性验证结果编排最终脚本：把每个镜头已找到的候选片段（一句话描述）交给
    编排 Agent，让它写最终字幕并选首选；每个 slot 的多候选原样继承下去，供后续 Split 挑不重复片段。

    shot_replicate=True（纯音乐/镜头复刻）：换用 ``orchestration_shot_replicate`` skill——尽量保持
    与参考相同的镜头数（不提前收尾）、每镜选动作最像参考的片段、口播留空。
    ...
    if shot_replicate:
        sk = get_skill("orchestration_shot_replicate")
        orch_prompt = sk.prompt_hint if (sk and sk.prompt_hint) else _FROM_FEAS_PROMPT
        skill_name = (sk.name if sk else "") or "镜头复刻编排"
    else:
        orch_prompt = _FROM_FEAS_PROMPT
    ...
    content = ""
    try:
        async for item in as_core.stream(orch_prompt, user):
    ...
    used_assets: set = set()   # 跨 slot 去重：已被前面 slot 选用的素材片段 asset_id
    for idx, shot in enumerate(shots):
        sid = shot.get("id")
        ...
        # 选片优先用**验证阶段（审核 Agent 仲裁后）已去重的 matched_asset_id**；其次编排 Agent 的
        # primary_asset_id；再退化 cands[0]。
        chosen_id = feas.get("matched_asset_id") or out.get("primary_asset_id")
        primary = next((c for c in cands if c["asset_id"] == chosen_id), None) \
            or next((c for c in cands if c["asset_id"] == out.get("primary_asset_id")), None) \
            or (cands[0] if cands else None)
        # 跨 slot 兜底去重：该片段已被前面的 slot 用了 → 改选本镜候选里第二相关（未用过的）
        if primary and primary["asset_id"] in used_assets:
            alt = next((c for c in cands if c["asset_id"] not in used_assets), None)
            if alt:
                primary = alt
        caption = (out.get("caption") or "").strip() or _shot_caption(shot)
        if feas.get("status") in ("direct", "partial") and primary:
            use_count += 1
            used_assets.add(primary["asset_id"])
        ...
        elif missing_shot_mode == "skip":
            # 缺失镜头处理=跳过：从成片里丢掉该镜（不进最终脚本）
            if kept_shots and kept_shots[-1] is shot:
                kept_shots.pop()
            dropped_slot_ids.append(sid_str)
            continue
        elif missing_shot_mode == "match" and primary:
            # 缺失镜头处理=匹配最相关：即便可行性判定不足，也强制用检索到最接近的片段
            use_count += 1
            used_assets.add(primary["asset_id"])
            alternates = [c for c in cands if c["asset_id"] != primary["asset_id"]]
            decisions[sid] = {
                "shot_id": sid, "status": "partial",
                "matched_asset_id": primary["asset_id"],
                "matched_source_video_id": primary.get("source_video_id", ""),
                "matched_source_path": primary.get("source_path", ""),
                "matched_time_range": primary.get("source_time_range", ""),
                ...
                "replicable_part": feas.get("replicable_part", ""),
                "reason": "缺失镜头按「匹配最相关」强制选用最接近的用户片段",
                "caption": caption, "alternates": alternates, "forced_match": True,
            }
            _set_shot_caption(shot, caption)
        else:
            # 缺失镜头处理=AIGC 生成（默认），或 match 无候选可用
            decisions[sid] = {
...
async def orchestrate_structure_first(reference_template: dict, material_understanding: dict, *,
                                      scheme_name="", material_strategy="faithful",
                                      selected_dimensions=None, selected_trends=None,
                                      intent="", missing_shot_mode="aigc") -> AsyncGenerator[dict, None]:
    ...
    for idx, spec in enumerate(slot_specs):
        ...
        use_this = bool(valid) and (action == "use_user_asset" or missing_shot_mode == "match")
        if use_this:
            primary = valid[0]
            alternates = valid[1:]
            user_speech = primary.get("speech_or_text") or ""
            final_caption = user_speech or caption
            forced = action != "use_user_asset"  # 本是 AIGC，但按「匹配最相关」强制用素材
            decisions[sid_int] = {
                ...
                "score": 1.0,
                "replicable_part": "",
                "reason": ("缺失镜头按「匹配最相关」强制选用最接近的用户片段" if forced
                           else (primary.get("reason") or spec.get("reason", "结构优先编排首选"))),
                "caption": final_caption,
                "alternates": alternates, "forced_match": forced,
            }
            ...
            # 缺失镜头处理=跳过：丢掉该 slot（不进最终结构）
            if new_slots and new_slots[-1].get("id") == sid_int:
                new_slots.pop()
            continue
        else:
            decisions[sid_int] = {
