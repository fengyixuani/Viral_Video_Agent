...
def export_scripts(*, project_name: str, template: dict, schemes: list, scheme_name: str,
    ...
    # 每次编排 = 一个独立、可读命名的 run 目录（outputs/{时间}_replicate_{项目}_{rid}/），
    # 产物集中在里面，不再往同一个项目目录里堆时间戳文件。
    run = runs.new_run("replicate", project_name)
    root = output_root or run["dir"]
    ...
    edit_plan_path = os.path.join(root, "asset_guided_edit_plan.json")
    ...
    # 契约②边界自检（生产端）：字段不符只告警、不打断，便于 A 尽早发现改坏了给 B 的接口
    _sp = contracts.validate_strategy(strategy)
    _log.log(30 if _sp else 20, "[contract] EditingStrategy(v%s): %s",
             contracts.CONTRACT_VERSION, contracts.summarize(_sp))
    strategy_path = os.path.join(root, "selected_editing_strategy.json")
    ...
    _cp = contracts.validate_connector_context(context)
    _log.log(30 if _cp else 20, "[contract] ConnectorContext: %s", contracts.summarize(_cp))
    ...
    # 登记到 outputs/index.json（最新在前），供「运行记录」检索
    runs.record(run, project=project_name, scheme=scheme_name,
                strategy_path=strategy_path, edit_plan_path=edit_plan_path,
                shots=len((strategy.get("editing_timeline") or [])))
    return {
        "dir": root,
